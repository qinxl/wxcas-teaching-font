...
import qs from 'qs';
const config = {
    PO_CODE: "http://127.0.0.1:57070/", // 本地 POCService 服务地址
    ROOT_PATH: "http://127.0.0.1/" // 后台地址
};
...
function po_md5_cmn(q, a, b, x, s, t) { return po_safe_add(po_bit_rol(po_safe_add(po_safe_add(a, q), po_safe_add(x, t)), s), b); }

function po_md5_ff(a, b, c, d, x, s, t) { return po_md5_cmn((b & c) | ((~b) & d), a, b, x, s, t); }

function po_md5_gg(a, b, c, d, x, s, t) { return po_md5_cmn((b & d) | (c & (~d)), a, b, x, s, t); }

function po_md5_hh(a, b, c, d, x, s, t) { return po_md5_cmn(b ^ c ^ d, a, b, x, s, t); }

function po_md5_ii(a, b, c, d, x, s, t) { return po_md5_cmn(c ^ (b | (~d)), a, b, x, s, t); }

function po_safe_add(x, y) { var lsw = (x & 0xFFFF) + (y & 0xFFFF); var msw = (x >> 16) + (y >> 16) + (lsw >> 16); return (msw << 16) | (lsw & 0xFFFF); }

function po_bit_rol(num, cnt) { return (num << cnt) | (num >>> (32 - cnt)); }

function po_str2binl(str) { var bin = []; var mask = (1 << 8) - 1; for (var i = 0; i < str.length * 8; i += 8) bin[i >> 5] |= (str.charCodeAt(i / 8) & mask) << (i % 32); return bin; }

function po_binl2hex(binarray) { var hex_tab = "0123456789ABCDEF"; var str = ""; for (var i = 0; i < binarray.length * 4; i++) { str += hex_tab.charAt((binarray[i >> 2] >> ((i % 4) * 8 + 4)) & 0xF) + hex_tab.charAt((binarray[i >> 2] >> ((i % 4) * 8)) & 0xF); } return str; }

function po_hex_md5(s) { return po_binl2hex(po_core_md5(po_str2binl(s), s.length * 8)); }
var bPOIsInstalled = false;
var POParent = po_hex_md5(window.location.pathname);
var PO_code = config.PO_CODE;
var PO_code2 = config.PO_CODE;
var PO_datas;
var POBrowser = {
    isChromeAndGreaterThan42: function () { var e = "42"; return this.getChromeVersion() >= e ? !0 : !1; },
    getChromeVersion: function () {
        var t = navigator.userAgent.toLowerCase(),
            n = /chrome/,
            o = /safari\/\d{3}\.\d{2}$/,
            i = /chrome\/(\S+)/;
        return n.test(t) && o.test(t) && i.test(t) ? RegExp.$1 : 0;
    },
    isChrome: function () {
    ...
    isOldIE: function () { var e = navigator.userAgent.toLowerCase(); return /msie/.test(e); },
    isXDR: function () {
        return false;
    },
    checkPOBrowserSate: function () { },
    strToHexCharCode: function (str) { if (str === "") return ""; var hexCharCode = []; for (var i = 0; i < str.length; i++) { hexCharCode.push((str.charCodeAt(i)).toString(16)); } return hexCharCode.join("").toUpperCase(); },
    checkSSL: function () {
        var strhref = window.location.href;
        strhref = strhref.toLowerCase();
        if (strhref.substr(0, 8) === "https://") { PO_code = PO_code2; }
        return true;
    },
    getRootPath: function () {
        return config.ROOT_PATH;
    },
    isInstalled: function () { return bPOIsInstalled; },
    showInstallDlg: function () { if (window.confirm("您需要安装PageOffice来打开文档。现在立即安装PageOffice吗？\r\n注意：安装完成后，请重新访问当前页面。")) { window.location.href = this.getRootPath() + "/posetup.exe"; } },
    openWindow: function (strURL, strOptions, strArgument) {
        let _this = this;
        if ((strURL === null) || (strURL === "")) { alert("The parameter strURL of openWindow() cannot be null or empty."); return; }
        if (!bPOIsInstalled) { this.showInstallDlg(); return; }
        var pathName = window.location.href, index;
        if (strURL.charAt(0) !== '/') {
            ...
            index = pathName.indexOf(window.location.pathname);
            strURL = pathName.substr(0, index) + strURL;
        }
        if ((strOptions !== null) && (strOptions[strOptions.length - 1] !== ';')) strOptions = strOptions + ";";
        axios.post(
            this.getRootPath() + "/poserver.zz?" + qs.stringify({ Info: "PageOfficeLink", pageurl: strURL, options: strOptions, params: strArgument })
        ).then(({ data }) => {
            PO_datas = data.split("\r\n");
            setTimeout(_this.sendUserdata(), 50);
            window.location.href = PO_datas[0] + POParent;
            jQuery().showPobDlg();
        });
        // jQuery.post(this.getRootPath() + "/poserver.zz", { Info: "PageOfficeLink", pageurl: strURL, options: strOptions, params: strArgument }, function (data) {
    ...
    openWindowModeless: function (strURL, strOptions, strArgument) {
        let _this = this;
        if ((strURL === null) || (strURL === "")) { alert("The parameter strURL of openWindowModeless() cannot be null or empty."); return; }
        if (!bPOIsInstalled) { this.showInstallDlg(); return; }
        var pathName = window.location.href, index;
        if (strURL.charAt(0) !== '/') {
            var strLower = strURL.toLowerCase();
        ...
        if ((strOptions !== null) && (strOptions[strOptions.length - 1] !== ';')) strOptions = strOptions + ";";
        jQuery.post(this.getRootPath() + "/poserver.zz", { Info: "PageOfficeLink", pageurl: strURL, options: strOptions + "IsModal=false;", params: strArgument }, function (data) {
            PO_datas = data.split("\r\n");
            ...
            jQuery.ajax({
                ...
                dataType: "text",
                global: false,
                error: function (XMLHttpRequest, textStatus, errorThrown) { if (textStatus === "timeout") { } else { } },
                success: function (data, textStatus) {
                    if (textStatus === "success") {
                        if (data === "") {
                            ...
                            setTimeout(_this.sendUserdata, 50);
                            window.location.href = PO_datas[0] + POParent + "|" + strToken;
                        }
                    }
                }
            });
    ...
    sendUserdata: function () {
        jQuery.post(PO_code + "userdata.htm?x=" + po_uuid(8, 16), { parent: POParent, Info: PO_datas[1] }, function (data) { }, "text");
    },
    getArgument: function () { try { return window.external.UserParams; } catch (e) { alert(e.message + ' Please ensure that you call it in POBrowser.'); } },
    callParentJs: function (strFunc) {
        if ((strFunc === null) || (strFunc === "")) { alert("The parameter strFunc of callParentJs() cannot be null or empty."); return; }
        try {
            ...
            return strRet;
        } catch (e) { alert(e.message + ' Please ensure that you call it in POBrowser.'); }
    },
    closeWindow: function () { try { return window.external.Close(); } catch (e) { alert(e.message + ' Please ensure that you call it in POBrowser.'); } },
    callback2: function () {
        let _this = this;
        jQuery.ajax({
            ...
            timeout: 120000,
            global: false,
            error: function (XMLHttpRequest, textStatus, errorThrown) { if (textStatus === "timeout") { setTimeout(_this.callback2(), 300); } else { setTimeout(_this.callback2(), 300); } },
            success: function (data, textStatus) {
                if ((textStatus === "success") && (data !== "null") && (data !== "abort")) {
                    ...
                    jQuery.post(PO_code + "funcret0.htm?x=" + po_uuid(8, 16), { id: parsedData[0].id }, function (data) {
                        var vRet;
                        try { vRet = _this.evil(parsedData[0].name); } catch (e) { alert(e.message); }
                        if (typeof (vRet) !== "string") { vRet = "undefined"; }
                        jQuery.post(PO_code + "funcret.htm?x=" + po_uuid(8, 16), { id: parsedData[0].id, ret: vRet }, function (data) { }, "text");
                    }, "text");
                }
                if (data !== "abort") setTimeout(_this.callback2, 300);
            }
        });
    },
    evil: function (fn) {
    ...
    resumePO: function () { jQuery.ajax({ url: PO_code + "resume.htm?parent=" + POParent + "&callback=?", dataType: "json", global: false, error: function (XMLHttpRequest, textStatus, errorThrown) { if (textStatus === "timeout") { } else { } }, success: function (data, textStatus) { if (textStatus === "success" && data) { } } }); }
};
jQuery(function () {
    ...
    dialogPob.find('.close').click(function () { dialogPob.trigger('hide'); });
    dialogPob.click(function (e) { });
    dialogPob.on('hide', function () { dialogPob.fadeOut(); }).on('show', function () { dialogPob.fadeIn(); });
    jQuery.fn.showPobDlg = function () { dialogPob.trigger('show'); };
    jQuery.fn.hidePobDlg = function () { dialogPob.trigger('hide'); };
});
